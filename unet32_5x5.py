...
class Downsample(nn.Module):
    def __init__(self, C):
        ...
        self.conv = nn.Conv2d(C, C, 5, stride=2, padding=2)
...
class Upsample(nn.Module):
    def __init__(self, C):
        ...
        self.conv = nn.Conv2d(C, C, 5, stride=1, padding=2)
...
class ResNetBlock(nn.Module):
    def __init__(self, in_ch, out_ch, dropout_rate=0.1):
        ...
        self.conv1 = nn.Conv2d(in_ch, out_ch, 5, stride=1, padding=2)
        self.dense = nn.Linear(512, out_ch)
        self.conv2 = nn.Conv2d(out_ch, out_ch, 5, stride=1, padding=2)
        if not (in_ch == out_ch):
            self.nin = Nin(in_ch, out_ch)
        self.dropout_rate = dropout_rate
        self.nonlinearity = torch.nn.SiLU()

    def forward(self, x, temb, demb):
        h = self.nonlinearity(nn.functional.group_norm(x, num_groups=32))
        h = self.conv1(h)
        h += self.dense(self.nonlinearity(temb))[:, :, None, None]
        h += self.dense(self.nonlinearity(demb))[:, :, None, None]
...
class UNet(nn.Module):
    def __init__(self, ch=128, in_ch=1):
        ...
        self.conv1 = nn.Conv2d(in_ch, ch, 5, stride=1, padding=2)
        ...
        self.up = nn.ModuleList([ResNetBlock(4 * ch, 2 * ch),
                                 ...
                                 ResNetBlock(2 * ch, ch),
                                 ResNetBlock(2 * ch, ch)])
        self.final_conv = nn.Conv2d(ch, in_ch, 5, stride=1, padding=2)
    ...
    def forward(self, x, t, params):
        ...
        demb = self.param_embedding(params.float())
        assert demb.shape == (params.shape[0], self.ch * 4)

        x1 = self.conv1(x)
        x2 = self.down[0](x1, temb, demb)
        x3 = self.down[1](x2, temb, demb)
        x4 = self.down[2](x3)
        x5 = self.down[3](x4, temb, demb)
        x6 = self.down[4](x5)
        x7 = self.down[5](x6, temb, demb)
        x8 = self.down[6](x7)
        x9 = self.down[7](x8)
        x10 = self.down[8](x9, temb, demb)
        x11 = self.down[9](x10, temb, demb)
        x12 = self.down[10](x11)
        x13 = self.down[11](x12, temb, demb)
        x14 = self.down[12](x13, temb, demb)

        x = self.middle[0](x14, temb, demb)
        x = self.middle[1](x)
        x = self.middle[2](x, temb, demb)

        x = self.up[0](torch.cat((x, x14), dim=1), temb, demb)
        x = self.up[1](torch.cat((x, x13), dim=1), temb, demb)
        x = self.up[2](torch.cat((x, x12), dim=1), temb, demb)
        x = self.up[3](x)
        x = self.up[4](torch.cat((x, x11), dim=1), temb, demb)
        x = self.up[5](torch.cat((x, x10), dim=1), temb, demb)
        x = self.up[6](torch.cat((x, x9), dim=1), temb, demb)
        x = self.up[7](x)
        x = self.up[8](torch.cat((x, x8), dim=1), temb, demb)
        x = self.up[9](x)
        x = self.up[10](torch.cat((x, x6), dim=1), temb, demb)
        x = self.up[11](x)
        x = self.up[12](torch.cat((x, x4), dim=1), temb, demb)
        x = self.up[13](x)
        x = self.up[14](x)
        x = self.up[15](torch.cat((x, x3), dim=1), temb, demb)
        x = self.up[16](torch.cat((x, x2), dim=1), temb, demb)
        x = self.up[17](torch.cat((x, x1), dim=1), temb, demb)
