...
class ResNetBlock(nn.Module):
    ...
    def forward(self, x, temb, demb):
        ...
        h = self.conv1(h)
        # add in timestep and digit embedding
...
class UNet(nn.Module):
    ...
    def forward(self, x, t, digit):
        ...
        :param digit: (torch.Tensor) tensor of digit labels (torch.long) [B]
        """
        ...
        demb = torch.nn.functional.one_hot(digit, num_classes=10).float()
        demb = self.digit_embedding(demb)
        assert demb.shape == (digit.shape[0], self.ch * 4)
